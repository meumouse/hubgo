...
/**
 * Environment snapshot rendered by the "Sobre" tab.
 *
...
class System_Status {
    ...
    public static function get_status() {
        ...
        $rows = array(
            self::row( 'plugin_version', esc_html__( 'Versão do HubGo', 'hubgo' ), defined( 'HUBGO_VERSION' ) ? HUBGO_VERSION : '—' ),
            ...
            self::row(
                ...
                defined( 'WC_VERSION' ) ? WC_VERSION : esc_html__( 'Não detectado', 'hubgo' ),
                defined( 'WC_VERSION' ) ? 'success' : 'danger'
            ),
            self::row(
                'hpos',
                esc_html__( 'Tabelas de pedidos (HPOS)', 'hubgo' ),
                self::is_hpos_enabled() ? esc_html__( 'Ativado', 'hubgo' ) : esc_html__( 'Desativado', 'hubgo' )
            ),
            self::row(
                'ssl',
                esc_html__( 'HTTPS', 'hubgo' ),
                is_ssl() ? esc_html__( 'Ativo', 'hubgo' ) : esc_html__( 'Inativo', 'hubgo' ),
                is_ssl() ? 'success' : 'warning'
            ),
            self::row(
                'wp_debug',
                esc_html__( 'WP_DEBUG', 'hubgo' ),
                ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ? esc_html__( 'Ativado', 'hubgo' ) : esc_html__( 'Desativado', 'hubgo' )
            ),
            self::row( 'memory_limit', esc_html__( 'Limite de memória', 'hubgo' ), (string) ini_get( 'memory_limit' ) ),
            self::row( 'max_execution_time', esc_html__( 'Tempo máximo de execução', 'hubgo' ), (string) ini_get( 'max_execution_time' ) . 's' ),
            self::row( 'locale', esc_html__( 'Idioma do site', 'hubgo' ), function_exists( 'determine_locale' ) ? determine_locale() : get_locale() ),
        );
