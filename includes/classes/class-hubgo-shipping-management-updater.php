 ...
 */
if ( !class_exists('Hubgo_Shipping_Management_Updater') ) {
    class Hubgo_Shipping_Management_Updater {
        ...
        public function __construct() {
            if ( defined( 'HUBGO_SHIPPING_MANAGEMENT_DEV_MODE' ) ) {
                add_filter( 'https_ssl_verify', '__return_false' );
                add_filter( 'https_local_ssl_verify', '__return_false' );
                add_filter( 'http_request_host_is_external', '__return_true' );
            }

            $this->plugin_slug = HUBGO_SHIPPING_MANAGEMENT_SLUG;
            $this->version = HUBGO_SHIPPING_MANAGEMENT_VERSION;
        ...
        public function request() {
            ...
            if ( false === $cached_data ) {
                ...
                if ( false === $remote ) {
                    $remote = wp_remote_get('https://github.com/meumouse/hubgo/raw/main/updater/hubgo-update-checker.json', [
                        'timeout' => 10,
                        'headers' => [
        ...
        public function add_check_updates_link( $actions, $plugin_file ) {
            if ( $plugin_file === $this->plugin_slug . '/' . $this->plugin_slug . '.php' ) {
                $check_updates_link = '<a href="' . esc_url( add_query_arg( 'hubgo_shipping_management_check_updates', '1' ) ) . '">' . esc_html__( 'Verificar atualizações', 'hubgo-shipping-management-wc' ) . '</a>';
                $actions['hubgo_shipping_management_check_updates'] = $check_updates_link;
            }
        ...
        public function check_manual_update_query_arg( $plugins ) {
            if ( isset( $_GET['hubgo_shipping_management_check_updates'] ) && $_GET['hubgo_shipping_management_check_updates'] === '1' ) {
                ...
                if ( $remote_data ) {
                    ...
                    if ( version_compare( $current_version, $latest_version, '<' ) ) {
                        $message = esc_html__('Uma nova versão do plugin HubGo - Gerenciamento de Frete para WooCommerce está disponível.', 'hubgo-shipping-management-wc');
                        ...
                        $message = esc_html__('A versão do plugin HubGo - Gerenciamento de Frete para WooCommerce é a mais recente.', 'hubgo-shipping-management-wc');
                        $class = 'notice-success';
                    }
                } else {
                    $message = esc_html__('Não foi possível verificar atualizações para o plugin HubGo - Gerenciamento de Frete para WooCommerce.', 'hubgo-shipping-management-wc');
                    $class = 'notice-error';
                }
...
Hubgo_Shipping_Management_Updater::run();