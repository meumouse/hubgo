...
* Compatibilidade com o plugin oficial do Melhor Envio — tudo resolvido pelo lado do HubGo, sem alterar o plugin de terceiros:
    - O cartão da integração passa a apontar para o plugin oficial (melhor-envio-cotacao), com instalação em um clique a partir do repositório do WordPress, em vez de aguardar um plugin complementar da MeuMouse
    - A integração deixa de ser Pro e passa a ser gratuita, como a da Frenet: o que ela faz é impedir que dois plugins que a loja já usa se contradigam, e isso não deveria depender de licença. O selo Pro sai do cartão e a licença deixa de ser exigida em tempo de execução
    - Correção: a calculadora do HubGo não funcionava com o Melhor Envio ativo. O plugin cota o carrinho do cliente, a menos que o pacote se identifique como cotação de página de produto; sem isso, ou a cotação saía com os itens errados (carrinho preenchido), ou a requisição quebrava ao ler um dado que não existia no pacote (carrinho vazio). O pacote do HubGo passa a ser montado no formato que o plugin espera (product_page_calculation e formatted_data), usando a própria fábrica de produtos do Melhor Envio — o que mantém kits e produtos compostos normalizados do mesmo jeito. Esta correção vale sempre que o plugin do Melhor Envio está ativo, mesmo com a integração desligada: é o que mantém a calculadora do HubGo de pé, não um recurso da integração
    - Novo: a transportadora que realmente move a encomenda (Correios, Jadlog, Loggi, Azul) passa a ser gravada como meta padrão da forma de entrega (carrier), o que faz o nome da transportadora — e não o do intermediário — aparecer nas notificações e no rastreio. O prazo de entrega já era lido do que o plugin grava em delivery_time
    - A opção "Importar rastreio" passa a ter efeito real: assim que o Melhor Envio grava o código de rastreio no pedido, ele é importado para o rastreio do HubGo com o Melhor Envio como provedor (para o link) e a transportadora cotada como nome exibido, disparando os e-mails e as automações do Joinotify como um código digitado à mão. Códigos anteriores à ativação da integração continuam sendo exibidos a partir do dado do outro plugin, somente leitura
    - A opção "Marcar como enviado" passa a ter efeito real: o pedido vai para o status "Pedido enviado" na primeira vez que um código do Melhor Envio chega, uma única vez por pedido, sem mexer em pedidos cancelados, reembolsados ou com falha
    - Correção: a página de produto exibia duas calculadoras. O simulador do plugin do Melhor Envio passa a ser removido quando a calculadora do HubGo está ativa, com nova opção "Ocultar simulador do Melhor Envio" no cartão da integração
    - Correção: "Marcar como enviado" não funcionava sozinha. A opção só era considerada quando "Importar rastreio" também estava ligada, porque as duas dependiam do mesmo gancho e ele só era registrado pela primeira. Agora cada uma vale por si
    - Correção: kits e produtos compostos (WPC Product Bundles e WPC Composite Products) recebiam uma cotação errada. Fora do carrinho o plugin do Melhor Envio só enxerga as medidas do produto-pai, que normalmente estão vazias — tanto que o próprio simulador dele se recusa a cotar e pede para calcular no carrinho. As formas de entrega do Melhor Envio passam a ser omitidas da calculadora nesses casos, em vez de mostrar um preço que a loja não vai conseguir praticar. Novo filtro para lojas que preencheram as medidas do kit: Hubgo/Integrations/Melhor_Envio/Skip_Compositions
    - Correção: o prazo aparecia duas vezes na calculadora. O plugin repete a janela de entrega dentro do nome da forma de entrega ("Correios Pac (3 a 5 dias úteis)") além de gravá-la como meta, e o HubGo já transforma essa meta em data prometida — que passa a divergir do texto assim que a loja define um tempo de manuseio. O trecho entre parênteses passa a ser removido, apenas nos pacotes da calculadora do HubGo
    - Nova opção "URL de rastreio" no cartão da integração, com o Melhor Rastreio como padrão
    - Novos filtros: Hubgo/Integrations/Melhor_Envio/Plugin_Files, Hubgo/Integrations/Melhor_Envio/Package_Url, Hubgo/Integrations/Melhor_Envio/Skip_Compositions
