...
admin/                      All PHP backend code + Composer
  ...
  src/
    ...
    Views/                  Storefront/admin rendering
      ...
      Custom_Colors.php         Deprecated 3.1.0, kept for its published filters
...
Since 3.1.0 the storefront calculator is a Vite bundle like the admin screens (`src/entries/storefront.js` → `app/dist/storefront/`), resolved through the manifest by `Core\Assets::enqueue_frontend_assets()` and configured through `hubgo_front_params`. The vanilla `assets/front/` bundle it replaced was deleted.
